...
T = x(1) + x(2)*L + (x(3)*L.^2) + (x(4)*L.^3) + (x(5)*L.^4); % Non-linear variation in temperature equation
 
k1 = (1.37*10^10)*exp(-75000/(R*T)); % Reaction 1 rate constant
k2 = (1.19*10^17)*exp(-125000/(R*T)); % Reaction 2 rate constant
 
ra = -k1*Ca; % Rate equation for species A
rb = k1*Ca - k2*Cb; % Rate equation for species B
rc = k2*Cb; % Rate equation for species C
